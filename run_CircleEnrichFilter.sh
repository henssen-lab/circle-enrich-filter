...
## Run on LSF-based HPC cluster at MSK as:
## bsub -n10 -W 1:00 -R "rusage[mem=10]" -o outfile_circEnrich_Chr2ChrM.txt /path/to/run_CircleEnrichFilter_PE_May2020.sh <file.bam> <outfolder>
...
## java=/opt/common/CentOS_7/java/jdk1.8.0_131/bin/java # version 1.8 for compatibility with picard
## picard=/data/cer/shared/software/picard/picard.jar # using 2.18 on our cluster
## homer=/data/cer/shared/software/HOMER
## bedtools=/opt/common/CentOS_7/bedtools/bedtools-2.29.2/bin/bedtools
## pgltools=/home/kocher/Programs/pgltools/sh/pgltools # v2.2.0; https://github.com/billgreenwald/pgltools
...
mkdir $tagdir
makeTagDirectory $tagdir $inbam -format sam -mapq $qfilt -rmsoft

# Enriched regions file name:
peakfile=Regions_${inbambase}.txt

findPeaks $tagdir -o $peakfile -style histone -fdr $enrichFDR -regionRes 20
...
picard MergeSamFiles AS=true USE_THREADING=true OUTPUT=$vmerge INPUT=$v97f1 INPUT=$v145r2 INPUT=$v81r1 INPUT=$v161f2 INPUT=$vSplit
